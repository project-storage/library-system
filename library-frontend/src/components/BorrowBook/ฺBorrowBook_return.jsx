...
export default function BorrowBook_return() {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await BorrowService.getSearchBorrow(searchId);
      ...
      <div className="container w-[660px] mx-auto mt-8">
        <div className="bb-borrow-box border-2 border-black p-6">
          ...
          <form onSubmit={handleSearch} className="flex justify-center mb-3">
              <label htmlFor="m_user">รหัสหนังสือที่ต้องการคืน:</label>
              <input
                type="text"
                id="m_user"
                name="username"
                ...
                required
              />
              <button type="submit" className="mx-4 border-[1.5px] border-black px-4 font-medium rounded-md">
                ค้นหา
              </button>
            </form>
          ...
          {bookData && (
              <div className="w-[560px] mx-auto my-4">
                <div className="flex flex-col justify-center border-[1.5px] border-black">
                  <div className="flex">
                    ...
                    <input
                      ...
                      value={bookData.tb_borrow_book.b_id}
                      readOnly
                    />
                  </div>
                  {/* ต่อไปเรียกข้อมูลอื่นๆ ของหนังสือและผู้ยืมที่คืน */}
                </div>
              </div>
            )}
            <div className="flex flex-col justify-center border-[1.5px] border-black">
